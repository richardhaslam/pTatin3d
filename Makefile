...
TATIN_COMPILE.cu = nvcc -c -Xcompiler "$(PCC_FLAGS) $(CCPPFLAGS) $(TATIN_CFLAGS) $(TATIN_INC) $(CFLAGS) $(C_DEPFLAGS)"
...
$(BINDIR)/%.app : $(OBJDIR)/src/%.o | $$(@D)/.DIR
	$(call quiet,PCC_LINKER) $(TATIN_CFLAGS) -o $@ $^ $(PETSC_SNES_LIB) $(LIBZ_LIB) $(CUDA_LIB)
