...
TATIN_INSTALL_DIR := $(PWD)/$(PETSC_ARCH)

# Logic to copy makefile.arch to the $(TATIN_INSTALL_DIR) directory for savekeeping
MAKEFILE_ARCH_SAVE_LOC=$(TATIN_INSTALL_DIR)/makefile.arch.log
save_config: $(TATIN_INSTALL_DIR)/.DIR makefile.arch
...
OBJDIR    ?= $(TATIN_INSTALL_DIR)/obj
LIBDIR    ?= $(TATIN_INSTALL_DIR)/lib
BINDIR    ?= $(TATIN_INSTALL_DIR)/bin
INCDIRGEN ?= $(TATIN_INSTALL_DIR)/include_gen
...
PTH_CONF_FILE=$(TATIN_INSTALL_DIR)/pth.conf
...
$(BINDIR)/%.app : $(OBJDIR)/%.o | $$(@D)/.DIR
	$(call quiet,PCC_LINKER) $(PCC_LINKER_FLAGS) $(TATIN_CFLAGS) -o $@ $^ $(PETSC_SNES_LIB) $(TATIN_LIB)
#@mv $@ $(BINDIR)
