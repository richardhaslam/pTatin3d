...
def test() :
  ...
  def comparefunc(t) :
    ...
    t.compareFloatingPoint(re.escape('slab edge identified: pidx_w,rank_w'),tol)
    t.compareFloatingPoint(re.escape('slab edge range: W,Dn'),tol)
    t.compareFloatingPoint(re.escape('slab edge (back face) range: gmax(x)'),tol)
    t.compareInteger(      re.escape('slab edge (back face) identified: pidx_w,rank_w'),0)
    t.compareFloatingPoint(re.escape('slab edge (back face) range: W,Dn'),tol)
    t.compareFloatingPoint(re.escape('slab edge (front face) range: gmin(z)'),tol)
    t.compareInteger(      re.escape('slab edge (front face) identified: pidx_w,rank_w'),0)
    t.compareFloatingPoint(re.escape('slab edge (front face) range: W,Dn'),tol)
