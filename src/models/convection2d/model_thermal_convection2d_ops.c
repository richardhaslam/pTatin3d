...
//	ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_JMIN_LOC,0,BCListEvaluator_Y_Thermal_Convection2d,(void*)c);CHKERRQ(ierr);
...
//	ierr = DMDAVecTraverse3d_Interp_X_Thermal_Convection2d(dau,velocity,0,data);CHKERRQ(ierr);
...
*/	
	/*V and P*/
	stokes_pack = c->stokes_ctx->stokes_pack;			
//	ierr = Thermal_Convection2d_Output_VP(c,stokes_pack,X,prefix);CHKERRQ(ierr);
...
//		ierr = Thermal_Convection2d_Output_Temperature(c,volQ,daT,temperature,prefix);CHKERRQ(ierr);
	}

	ierr = DMCompositeGetEntries(stokes_pack,&dau,&dap);CHKERRQ(ierr);
	
//	ierr=Thermal_Convection2d_Output_Viscosity(c,dau,materialpoint_db,prefix);
...
PetscErrorCode pTatinModelRegister_Thermal_Convection2d(void)
{
	...
	ierr = pTatinModelSetName(m,"thermal_convection2d");CHKERRQ(ierr);
