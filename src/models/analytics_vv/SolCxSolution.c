...
PetscErrorCode SolCxSolution(const PetscReal pos[],
                   PetscReal _eta_A,PetscReal _eta_B,   /* Input parameters: viscosity A, viscosity B */
                   PetscReal _x_c, PetscInt _n,              /* Input parameters: viscosity jump location, wavenumber in z */
                   PetscReal vel[],
                   PetscReal *presssure,
                   PetscReal total_stress[],
                   PetscReal strain_rate[])
{
  ...
  if (total_stress) {
    total_stress[0] = sum3; /* xx */
    total_stress[1] = sum6; /* zz */
    total_stress[2] = sum4; /* xz */
  }
  if (strain_rate) {
    if (x>xc) Z = ZB;
    else Z = ZA;
