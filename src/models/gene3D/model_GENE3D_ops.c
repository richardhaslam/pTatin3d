...
ModelInitialize_GENE3D (pTatinCtx c, void *ctx)
{
  ...
  PetscPrintf (PETSC_COMM_WORLD,
	       "reading model initial geometry from options\n");
  ierr =
    PetscOptionsGetInt (PETSC_NULL, "-initial_geom",
			(PetscInt *) & data->initial_geom, &found);
  CHKERRQ (ierr);
  if (found == PETSC_FALSE)
    {
      SETERRQ (PETSC_COMM_SELF, PETSC_ERR_USER,
	       "Expected user to provide a type of material index initialisation \n");
    }
  ...
  if (found == PETSC_FALSE)
    {
      SETERRQ (PETSC_COMM_SELF, PETSC_ERR_USER,
	       "Expected user to provide model length Lz \n");
    }
  ...
  if (found == PETSC_FALSE)
    {
      SETERRQ (PETSC_COMM_SELF, PETSC_ERR_USER,
	       "Expected user to provide model Origin Oz \n");
    }
  ...
  ierr =
    PetscOptionsGetInt (model, "-rheol",
			(PetscInt *) & rheology->rheology_type, &found);
  CHKERRQ (ierr);
  if (found == PETSC_FALSE)
    {
      SETERRQ (PETSC_COMM_SELF, PETSC_ERR_USER,
	       "Expected user to provide value for rheology type \n");
    }

  /* material properties */
  PetscPrintf (PETSC_COMM_WORLD,
	       "reading material properties from options\n");
  ierr = PetscOptionsGetInt (model, "-nphase", &nphase, &found);
  CHKERRQ (ierr);
  if (found == PETSC_FALSE)
    {
      SETERRQ (PETSC_COMM_SELF, PETSC_ERR_USER,
	       "Expected user to provide value for number of materials \n");
    }
  for (i = 0; i < nphase; i++)
    {
	...
	case 1:
	  {
	    ...
	    ierr =
	      PetscOptionsGetReal (model, option_name,
				   &(rheology->mises_tau_yield[i]), &found);
	    CHKERRQ (ierr);
	    if (found == PETSC_FALSE)
	      SETERRQ1 (PETSC_COMM_SELF, PETSC_ERR_USER,
			"Expected user to provide value to option %s \n",
	    ...
	    ierr =
	      PetscOptionsGetReal (model, option_name,
				   &(rheology->dp_pressure_dependance[i]),
				   &found);
	    CHKERRQ (ierr);
	    if (found == PETSC_FALSE)
	      SETERRQ1 (PETSC_COMM_SELF, PETSC_ERR_USER,
	...
	case 2:
	  {
	    ...
	    ierr =
	      PetscOptionsGetReal (model, option_name,
				   &(rheology->soft_min_strain_cutoff[i]),
				   &found);
	    CHKERRQ (ierr);
	    if (found == PETSC_FALSE)
	      SETERRQ1 (PETSC_COMM_SELF, PETSC_ERR_SUP,
	    ...
	    ierr =
	      PetscOptionsGetReal (model, option_name,
				   &(rheology->soft_max_strain_cutoff[i]),
				   &found);
	    CHKERRQ (ierr);
	    if (found == PETSC_FALSE)
	      SETERRQ1 (PETSC_COMM_SELF, PETSC_ERR_SUP,
	...
	case 3:
	  {
	    SETERRQ (PETSC_COMM_SELF, PETSC_ERR_USER,
		     "visco-elasto-plastic rheology not handled in 3D tatin \n");
	  }
	}
    }
...
ModelApplyBoundaryCondition_GENE3D (pTatinCtx user, void *ctx)
{
  ...
  switch (data->boundary_conditon_type)
    ...
    case GENEBC_FreeSlip:
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_IMIN_LOC, 0,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_IMAX_LOC, 0,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);

      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_JMIN_LOC, 1,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_JMAX_LOC, 1,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);

      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_KMIN_LOC, 2,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_KMAX_LOC, 2,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      break;
	...
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
      ...
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_IMIN_LOC, 1,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_IMIN_LOC, 2,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
	...
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
      ...
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_IMAX_LOC, 1,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_IMAX_LOC, 2,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);

      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_JMIN_LOC, 0,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
      ...
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_JMIN_LOC, 2,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);

      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_JMAX_LOC, 0,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
      ...
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_JMAX_LOC, 2,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);

      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_KMIN_LOC, 0,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_KMIN_LOC, 1,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
      ...
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_KMAX_LOC, 0,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
			      user->stokes_ctx->dav, DMDABCList_KMAX_LOC, 1,
			      BCListEvaluator_constant, (void *) &zero);
      CHKERRQ (ierr);
      ierr =
	DMDABCListTraverse3d (user->stokes_ctx->u_bclist,
...
ModelApplyInitialMeshGeometry_GENE3D (pTatinCtx c, void *ctx)
{
  ...
  ierr =
    DMDASetUniformCoordinates (c->stokes_ctx->dav, data->Ox, data->Lx,
			       data->Oy, data->Ly, data->Oz, data->Lz);
  CHKERRQ (ierr);
...
/* define phase index on material points from a map file extruded in z direction */
{
  ...
  nLayer = 1;
  ierr = PetscOptionsGetInt ("model_GENE3D_", "-nlayer", &nLayer, &flg);
  CHKERRQ (ierr);
  YLayer[0] = data->Oy;
  for (i = 1; i < nLayer; i++)
    {
      ...
      ierr =
	PetscOptionsGetReal ("model_GENE3D_", option_name, &YLayer[i], &flg);
      CHKERRQ (ierr);
      if (flg == PETSC_FALSE)
	{
	  SETERRQ1 (PETSC_COMM_SELF, PETSC_ERR_USER,
      ...
      else
	{
	  SETERRQ (PETSC_COMM_SELF, PETSC_ERR_SUP,
		   "Layers must be entered so that ylayer_[i-1] is larger than ylayer_[i]\n");
	}
    }
...
/* define phase index on material points from a map file extruded in z direction */
{
  ...
  ierr =
    PetscOptionsGetString ("model_GENE3D_", "-map_file", map_file,
			   PETSC_MAX_PATH_LEN - 1, &flg);
  CHKERRQ (ierr);
  if (flg == PETSC_FALSE)
    {
      SETERRQ (PETSC_COMM_SELF, PETSC_ERR_USER,
	       "Expected user to provide a map file \n");
    }
  ...
  for (p = 0; p < n_mp_points; p++)
    {
      ...
      MPntPStokesSetField_eta_effective (mpprop_stokes,
					 rheology->const_eta0[phase_index]);
      MPntPStokesSetField_density (mpprop_stokes,
				   rheology->const_rho0[phase_index]);
    }
...
PetscErrorCode
ModelApplyInitialMaterialGeometry_GENE3D (pTatinCtx c, void *ctx)
{
  PetscErrorCode ierr;
  ModelGENE3DCtx *data = (ModelGENE3DCtx *) ctx;
  PetscFunctionBegin;
  ...
  switch (data->initial_geom)
    ...
    case 0:
      {
	ierr = ModelSetMarkerIndexLayeredCake_GENE3D (c, ctx);
	CHKERRQ (ierr);
      }
      break;
    ...
    case 1:
      {
	ierr = ModelSetMarkerIndexFromMap_GENE3D (c, ctx);
	CHKERRQ (ierr);
      }
      break;
      /*Read from CAD file */
    case 2:
      {
	SETERRQ (PETSC_COMM_SELF, PETSC_ERR_USER,
		 "Reading from CAD is not implemented yet \n");
      }
      break;
    }
  ierr = ModelSetInitialStokesVariableOnMarker_GENE3D (c, ctx);
  CHKERRQ (ierr);
...
ModelOutput_GENE3D (pTatinCtx c, Vec X, const char prefix[], void *ctx)
{
  ...
  ierr = pTatin3d_ModelOutput_VelocityPressure_Stokes (c, X, prefix);
  CHKERRQ (ierr);
  ierr = pTatin3d_ModelOutput_MPntStd (c, prefix);
  CHKERRQ (ierr);
...
pTatinModelRegister_GENE3D (void)
{
  ...
  ierr = PetscMalloc (sizeof (ModelGENE3DCtx), &data);
  CHKERRQ (ierr);
  ierr = PetscMemzero (data, sizeof (ModelGENE3DCtx));
  CHKERRQ (ierr);

  /* register user model */
  ierr = pTatinModelCreate (&m);
  CHKERRQ (ierr);

  /* Set name, model select via -ptatin_model NAME */
  ierr = pTatinModelSetName (m, "GENE3D");
  CHKERRQ (ierr);

  /* Set model data */
  ierr = pTatinModelSetUserData (m, data);
  CHKERRQ (ierr);

  /* Set function pointers */
  ierr =
    pTatinModelSetFunctionPointer (m, PTATIN_MODEL_INIT,
				   (void (*)(void)) ModelInitialize_GENE3D);
  CHKERRQ (ierr);
  ierr =
    pTatinModelSetFunctionPointer (m, PTATIN_MODEL_APPLY_BC,
				   (void (*)(void))
				   ModelApplyBoundaryCondition_GENE3D);
  CHKERRQ (ierr);
  ierr =
    pTatinModelSetFunctionPointer (m, PTATIN_MODEL_APPLY_MAT_BC,
				   (void (*)(void))
				   ModelApplyMaterialBoundaryCondition_GENE3D);
  CHKERRQ (ierr);
  ierr =
    pTatinModelSetFunctionPointer (m, PTATIN_MODEL_APPLY_INIT_MESH_GEOM,
				   (void (*)(void))
				   ModelApplyInitialMeshGeometry_GENE3D);
  CHKERRQ (ierr);
  ierr =
    pTatinModelSetFunctionPointer (m, PTATIN_MODEL_APPLY_INIT_MAT_GEOM,
				   (void (*)(void))
				   ModelApplyInitialMaterialGeometry_GENE3D);
  CHKERRQ (ierr);
  ierr =
    pTatinModelSetFunctionPointer (m, PTATIN_MODEL_APPLY_UPDATE_MESH_GEOM,
				   (void (*)(void))
				   ModelApplyUpdateMeshGeometry_GENE3D);
  CHKERRQ (ierr);
  ierr =
    pTatinModelSetFunctionPointer (m, PTATIN_MODEL_OUTPUT,
				   (void (*)(void)) ModelOutput_GENE3D);
  CHKERRQ (ierr);
  ierr =
    pTatinModelSetFunctionPointer (m, PTATIN_MODEL_DESTROY,
				   (void (*)(void)) ModelDestroy_GENE3D);
  CHKERRQ (ierr);

  /* Insert model into list */
  ierr = pTatinModelRegister (m);
  CHKERRQ (ierr);
