...
PetscErrorCode ModelApplyInitialMaterialGeometry_rift_fastscape_3D(pTatinCtx c,void *ctx)
{
	...
	for (p=0; p<n_mp_points; p++) {
		...
		if (notch_type == 1 ) {
			...
			// Crustal layer in which weak zone are defined
			if ((yp_dimensional < (notch_base + notch_height)) && (yp_dimensional >= notch_base)) {
				//Define front faults
				if (zp_dimensional >= 100e3 && zp_dimensional < 233e3){
					if ( abs( (xp_dimensional)-(200e3+yp_dimensional/(beta*convert)) ) <= 1e3 ){
						plastic_strain = 1.0;
					}
					if ( abs( (xp_dimensional)-(400e3+yp_dimensional/(beta*convert)) ) <= 1e3 ){
						plastic_strain = 1.0;
					}
				}
				//Define middle fault
				if (zp_dimensional >= 233e3 && zp_dimensional < 366e3){
					if ( abs( (xp_dimensional)-(300e3+yp_dimensional/(beta*convert)) ) <= 1e3 ){
						plastic_strain = 1.0;
					}
				}			
				//Define back faults
				if (zp_dimensional >= 366 && zp_dimensional < 500e3){
					if ( abs( (xp_dimensional)-(200e3+yp_dimensional/(beta*convert)) ) <= 1e3 ){
						plastic_strain = 1.0;
					}
					if ( abs( (xp_dimensional)-(400e3+yp_dimensional/(beta*convert)) ) <= 1e3 ){
