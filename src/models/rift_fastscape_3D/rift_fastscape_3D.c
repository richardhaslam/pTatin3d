...
PetscErrorCode ModelInitialize_rift_fastscape_3D(pTatinCtx c,void *ctx)
{
	...
	ierr = PetscOptionsGetReal(NULL,"-model_rift_fastscape_3D_rhowm",&data->rhowm,&flg);CHKERRQ(ierr);
	...
	ierr = PetscOptionsGetReal(NULL,"-model_rift_fastscape_3D_etawm",&data->etawm,&flg);CHKERRQ(ierr);
	...
	data->refinement_factor   = 2;
	data->dt_spm   = 100; //Fastscape timestep scaled
	data->_law   = 1;  
	data->_m   = 0.4;
	data->_kf   = 1e-5;
	data->_kd   = 0.; //Not used
	data->_bc   = 101; //Four digit number
	
 	ierr = PetscOptionsGetInt(NULL,"-model_rift_fastscape_3D_refinement_factor",&data->refinement_factor,&flg);CHKERRQ(ierr);
 	ierr = PetscOptionsGetReal(NULL,"-model_rift_fastscape_3D_dt_spm",&data->dt_spm,&flg);CHKERRQ(ierr);
 	ierr = PetscOptionsGetReal(NULL,"-model_rift_fastscape_3D_law",&data->_law,&flg);CHKERRQ(ierr);
	ierr = PetscOptionsGetInt(NULL,"-model_rift_fastscape_3D_m",&data->_m,&flg);CHKERRQ(ierr);
 	ierr = PetscOptionsGetReal(NULL,"-model_rift_fastscape_3D_kf",&data->_kf,&flg);CHKERRQ(ierr);
 	ierr = PetscOptionsGetReal(NULL,"-model_rift_fastscape_3D_kd",&data->_kd,&flg);CHKERRQ(ierr);
	ierr = PetscOptionsGetInt(NULL,"-model_rift_fastscape_3D_bc",&data->_bc,&flg);CHKERRQ(ierr);
	...
	if(use_energy) {
		...
		data->thermalparams.ytop[0]  = 500.0e3;
		...
		data->thermalparams.ytop[1]  = 465.0e3;
		...
		data->thermalparams.ytop[2]  = 380.0e3;
		data->thermalparams.thick[2] = 380.0e3;
	...
	//----------------------------------//
	/* PHASE 2, WEAK MANTLE LITHOSPHERE */
	//----------------------------------//
	...
	//VISCOSITY PARAMETERS - wet olivine x 5, karato
	preexpA = 1.393e-14;
	Ascale  = 0.5; //0,1 * wet olovine rheology to trigger delamination
	entalpy = 430.0e3;
	Vmol    = 15.0e-6;
	nexp    = 3.0;
	Tref    = 273.0;
	...
	//DENSITY PARAMETERS FOR STOKES
	MaterialConstantsSetValues_DensityBoussinesq(materialconstants,regionidx,data->rhowm,2.e-5,0.0);
  ...
  /* ENERGY FLOW LAW DEFS */
  {
    ...
    EnergySourceConstSetField_HeatSource(&matconstants_h_const[2],0.0);
    ...
    //UPPER CRUST MARKERS
    MaterialConstantsSetValues_EnergyMaterialConstants(6,matconstants_e,alpha,beta,rho_ref,Cp,density_type,conductivity_type,NULL);
    EnergyConductivityConstSetField_k0(&matconstants_k_const[6],k);
    EnergySourceConstSetField_HeatSource(&matconstants_h_const[6],0.9e-6);
	...
	PetscPrintf(PETSC_COMM_WORLD,"[rift_fastscape_3D]  -model_rift_fastscape_3D_rhowm [kg/m^3] :%+1.4e \n", data->rhowm );
	...
	if (data->dimensional) {
		...
		data->rhowm = data->rhowm/data->density_bar;
  ...
  //  printf("[crust phase 2] kappa %1.9e H %1.9e\n",
   ...
   // 0.9e-6*data->time_bar/data->pressure_bar);
      ...
      MaterialConstantsEnergyScaleAll(materialconstants,regionidx,data->length_bar,data->time_bar,data->pressure_bar);
		...
		PetscPrintf(PETSC_COMM_WORLD,"[rift_fastscape_3D] scaled value   -model_rift_fastscape_3D_rhowm:%+1.4e \n", data->rhowm );
...
PetscErrorCode ModelApplyInitialMaterialGeometry_rift_fastscape_3D(pTatinCtx c,void *ctx)
{
	...
	for (p=0; p<n_mp_points; p++) {
		...
		double      *position;
		double      eta,rho,xref,xrefL,xrefR,xrefF,xrefB;
		int         phase;
		...
		if ((position[1] > data->ha) && (position[1] < (data->ha+data->hm))) {
			//if ( (position[0]*data->length_bar) > 200e3 && (position[0]*data->length_bar) < 600e3 ){
			xrefL = 250e3 + fabs( ((position[1]*data->length_bar) - (data->ha*data->length_bar))/2. );
			xrefR = 750e3 - fabs( ((position[1]*data->length_bar) - (data->ha*data->length_bar))/2. );
			xrefF = 200e3 + fabs( ((position[1]*data->length_bar) - (data->ha*data->length_bar))/2. );
			xrefB = 800e3 - fabs( ((position[1]*data->length_bar) - (data->ha*data->length_bar))/2. );
			if ( (position[0]*data->length_bar) > xrefL && (position[0]*data->length_bar) < xrefR && (position[2]*data->length_bar) > xrefF && (position[2]*data->length_bar) < xrefB){
				phase = 2;
				eta=data->etawm;
				rho=data->rhowm;
			}
		...
		if (notch_type == 1 ) {
			...
			if (yp_dimensional >= notch_base) {
				/*if (zp_dimensional >= 300e3 && zp_dimensional < 400e3){
					xref = 400e3 + (yp_dimensional-notch_base)/2.;
					if ( fabs(xp_dimensional - xref) <= fault_width) {
						plastic_strain = 1.0;
						phase = 4;
					}
					xref = 600e3 + (yp_dimensional-notch_base)/2.;
					if ( fabs(xp_dimensional - xref) <= fault_width) {
						plastic_strain = 1.0;
						phase = 4;
					}				
				...
				if (zp_dimensional >= 450e3 && zp_dimensional < 550e3){
					xref = 500e3 + (yp_dimensional-notch_base)/2.;
					if ( fabs(xp_dimensional - xref) <= fault_width) {
						plastic_strain = 1.0;
						phase = 4;
					}	
				}
				if (zp_dimensional >= 600e3 && zp_dimensional < 700e3){
					xref = 400e3 + (yp_dimensional-notch_base)/2.;
					if ( fabs(xp_dimensional - xref) <= fault_width) {
						plastic_strain = 1.0;
						phase = 4;
				...
				if (zp_dimensional >= 0. && zp_dimensional < 1000e3){
					xref = 500e3 + (yp_dimensional-notch_base)/2.;
					if ( fabs(xp_dimensional - xref) <= fault_width) {
						plastic_strain = 1.0;
						phase = 4;
...
PetscErrorCode ModelApplyExportInnerMesh_rift_fastscape_3D(pTatinCtx c,Vec X,void *ctx)
{
	...
	if (rank == 0) {
		...
		/* Other ranks must grab 1 point due to design limitation of the DMDA */
		/* This "one point" is will not be used in any computations */
		/* We request it grabs a locally owned point to avoid excessive communication */
		si = lsi;
		sj = lsj;
		sk = lsk;
		ei=si+1;
...
PetscErrorCode ModelApplyUpdateMeshGeometry_rift_fastscape_3D_semi_eulerian(pTatinCtx c,Vec X,void *ctx)
{
	...
	/* SURFACE PROCESSES: FASTSCAPE */
	/*
	PetscReal dt_sp;
	dt_sp = step/100.;
	ierr = ptatin3d_ApplyLandscapeEvolutionModel_FastScape_V3(c,X,data->refinement_factor,data->length_bar,data->velocity_bar,step,dt_sp,data->_law,data->_m,data->_kf,data->_kd,data->_bc);CHKERRQ(ierr);
*/
