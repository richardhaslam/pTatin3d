...
PetscErrorCode ModelInitialize_Ridge_transform(pTatinCtx c,void *ctx)
{
	...
	data->vx_up = 3.8 / 2.0 * cm_per_yer2m_per_sec;
	...
	if(use_energy) {
		...
		data->thermalparams.ytop[0]  = 34.0e3;
		data->thermalparams.thick[0] = 34.0e3;
		...
		data->thermalparams.vexp[0]  = 1.96 * cm_per_yer2m_per_sec; //expension velocity in cm/yr
