...
PetscErrorCode DMDAComputeMeshQualityMetric_FaceAreaRatio(DM dm,PetscReal *value)
{
	...
	for (e=0;e<nel;e++) {
		...
		get_face_corners_coordinates(el_coords, Q2_FACE_WEST, corners);
        a1 = compute_quadrilateral_area_(corners);
        get_face_corners_coordinates(el_coords, Q2_FACE_EAST, corners);
		a2 = compute_quadrilateral_area_(corners);
        ratio = (a1<a2)?a1/a2:a2/a1;
        ratio_min = (ratio<ratio_min)?ratio:ratio_min;

		get_face_corners_coordinates(el_coords, Q2_FACE_SOUTH, corners);
        a1 = compute_quadrilateral_area_(corners);
        get_face_corners_coordinates(el_coords, Q2_FACE_NORTH, corners);
		a2 = compute_quadrilateral_area_(corners);
        ratio = (a1<a2)?a1/a2:a2/a1;
        ratio_min = (ratio<ratio_min)?ratio:ratio_min;        
        
		get_face_corners_coordinates(el_coords, Q2_FACE_BACK, corners);
        a1 = compute_quadrilateral_area_(corners);
        get_face_corners_coordinates(el_coords, Q2_FACE_FRONT, corners);
		a2 = compute_quadrilateral_area_(corners);
        ratio = (a1<a2)?a1/a2:a2/a1;
        ratio_min = (ratio<ratio_min)?ratio:ratio_min;            
	}
...
PetscErrorCode DMDAComputeMeshQualityMetric(DM dm,MeshQualityMeasure measure,PetscReal *value)
{
	...
	switch (measure) {
        ...
        case MESH_QUALITY_CORNER_ANGLE:
			...
			break;
        case MESH_QUALITY_FACEAREA_RATIO:
