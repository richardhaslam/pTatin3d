...
PetscErrorCode MarkerCoordinatesLoadFromFile(const char name[],long int *length,double **coords)
{
	...
	*length = n_markers;
	...
	for (p=0; p<n_markers; p++) {
		if (fread(&data[3*p],sizeof(double),3,fp) < 3) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_FILE_READ,"fread() failure.");
...
PetscErrorCode MarkerScalarFieldLoadFromFile(const char name[],long int *length,void **field)
{
	...
	*length = n_markers;
	...
	if (fread(data,datasize,n_markers,fp) < n_markers) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_FILE_READ,"fread() failure");
