...
PetscErrorCode DataExTopologyAddNeighbour( DataEx d, const PetscMPIInt proc_id )
{
	...
	if (proc_id<0) {
		SETERRQ(d->comm,PETSC_ERR_ARG_WRONGSTATE,"Trying to set proc neighbour with a rank < 0");
	}
	/* error on ranks larger than number of procs in communicator */
	MPI_Comm_size(d->comm,&nproc);
	if (proc_id>=nproc) {
		SETERRQ(d->comm,PETSC_ERR_ARG_WRONGSTATE,"Trying to set proc neighbour with a rank >= nproc");
	}
...
PetscErrorCode DataExAddToSendCount( DataEx de, const int proc_id, const int count )
{
	...
	if( local_val == -1 ) {
		SETERRQ1( de->comm, PETSC_ERR_ARG_WRONG,"Proc %d is not a valid neighbour rank", proc_id );
	}
...
PetscErrorCode _DataExInitializeTmpStorage( DataEx de )
{
	...
	if( de->n_neighbour_procs < 0 ) {
		SETERRQ( de->comm, PETSC_ERR_ARG_SIZ, "Number of neighbour procs < 0");
	}
	if( de->neighbour_procs == NULL ) {
		SETERRQ( de->comm, PETSC_ERR_ARG_NULL, "Neighbour proc list is NULL" );
	}
...
PetscErrorCode DataExPackData( DataEx de, int proc_id, int n, void *data )
{
	...
	if( local == -1 ) {
		SETERRQ1( de->comm, PETSC_ERR_ARG_WRONG, "proc_id %d is not registered neighbour", proc_id );
	}
	
	if( n+de->pack_cnt[local] > de->messages_to_be_sent[local] ) {
		SETERRQ3( de->comm, PETSC_ERR_ARG_WRONG, "Trying to pack too many entries to be sent to proc %d. Space requested = %d: Attempt to insert %d", 
...
PetscErrorCode DataExPackFinalize( DataEx de )
{
	...
	for( i=0; i<np; i++ ) {
		if( de->pack_cnt[i] != de->messages_to_be_sent[i] ) {
			SETERRQ3( de->comm, PETSC_ERR_ARG_WRONGSTATE, "Not all messages for neighbour[%d] have been packed. Expected %d : Inserted %d", 
