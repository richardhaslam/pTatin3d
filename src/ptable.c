...
PetscErrorCode  DMLocalToGlobalBegin_DAQ2(DM da,Vec l,InsertMode mode,Vec g)
{
  ...
  PetscObjectQuery((PetscObject)da,"g2lq2",(PetscObject*)&myg2l);
  if (mode == ADD_VALUES) {
    ierr = VecScatterBegin(myg2l,l,g,ADD_VALUES,SCATTER_REVERSE);CHKERRQ(ierr);
  } else if (mode == INSERT_VALUES) {
    ierr = VecScatterBegin(myg2l,l,g,INSERT_VALUES,SCATTER_REVERSE);CHKERRQ(ierr);
...
PetscErrorCode  DMLocalToGlobalEnd_DAQ2(DM da,Vec l,InsertMode mode,Vec g)
{
  ...
  PetscObjectQuery((PetscObject)da,"g2lq2",(PetscObject*)&myg2l);
  if (mode == ADD_VALUES) {
    ierr = VecScatterEnd(myg2l,l,g,ADD_VALUES,SCATTER_REVERSE);CHKERRQ(ierr);
  } else if (mode == INSERT_VALUES) {
    ierr = VecScatterEnd(myg2l,l,g,INSERT_VALUES,SCATTER_REVERSE);CHKERRQ(ierr);
  } else SETERRQ(PetscObjectComm((PetscObject)da),PETSC_ERR_SUP,"Not yet implemented");
  PetscFunctionReturn(0);
}

PetscErrorCode  DMCreateGlobalVector_DAQ2(DM da,Vec *g)
{
  PetscErrorCode ierr;
  VecScatter     myg2l = NULL;
  PetscInt       m,n,length;
  ...
  DMDAGetCorners(da,0,0,0,&m,&n,0);
  length = m*n;
  VecCreate(PetscObjectComm((PetscObject)da),g);
  VecSetSizes(*g,length,PETSC_DECIDE);
  VecSetFromOptions(*g);
...
PetscErrorCode dmdaq2(void)
{
  ...
  IS from,to;
  int rank;
  PetscInt lx[2],ly[1],N;
  Vec xl,xg;
  ISLocalToGlobalMapping l2gmap;
  const PetscInt *gindices;
  PetscInt i,j,nx_ghost;
  PTable table;
  long int cnt;
  VecScatter scatter;
  ...
  DMDACreate2d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_BOX,
               2*mx+1,2*mx+1,2,1,1,2,lx,ly,&da);

  DMCreateGlobalVector(da,&xg);
  DMCreateLocalVector(da,&xl);
  VecGetSize(xl,&N);
  printf("rank %d : <dm-default> N %d\n",rank,N);
  ...
  DMLocalToGlobalBegin(da,xl,ADD_VALUES,xg);
  DMLocalToGlobalEnd(da,xl,ADD_VALUES,xg);

  VecView(xg,PETSC_VIEWER_STDOUT_WORLD);

  DMGetLocalVector(da,&xl);
  DMRestoreLocalVector(da,&xl);
