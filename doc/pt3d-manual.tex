...
\begin{itemize}
	...
	\item git (non essential - used to obtain the source)
	\item \href{www.paraview.org}{ParaView} (essential for visualizing simulation output)
	\item Python 2.6-2.7 (to run the test suite)
\end{itemize}

\section{Obtaining the Code}
{\ptat} is available via git using the command
\shellcmd{git clone git@bitbucket.org:jedbrown/ptatin3d.git}
...
\end{itemize}
The following targets for make exist : \unix{all}, \unix{drivers}, \unix{tests}.
The make system will create all objects, libraries and binary files in the following paths: \newline
	\unix{ptatin3d/\$\{PETSC\_ARCH\}/obj} \newline
	\unix{ptatin3d/\$\{PETSC\_ARCH\}/lib} \newline
	\unix{ptatin3d/\$\{PETSC\_ARCH\}/bin} \newline
The current build system allows you to conveniently compile different {\ptat} using different pets builds (e.g. a debug versus optimised).
...
\begin{enumerate}
% -------
	\item The ideal choice of $x^*, v^*, \eta^*$ scaling should result in values of velocity and pressure which follow $\bar{v} = \max_k(\vert v'_k \vert) \sim 1$ and $\bar{p} = \max_k(\vert p'_k \vert) \sim 1$. If your scaling doesn't produce this type of behaviour, then one should be careful in choosing the stopping condition applied to both the non-linear and linear solvers. For example, if $\bar{v} \sim 10^6$, it does't make much sense to solve the non-linear equations to a tolerance of $F_u, F_p \sim 10^{-9}$.
% -------
